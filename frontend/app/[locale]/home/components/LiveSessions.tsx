          ...
          >
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-800 rounded-2xl text-indigo-600 dark:text-indigo-400">
                ...
                <span className="text-[10px] font-bold uppercase leading-none">{session.time.split(":")[1]}</span>
              </div>
              <div>
            ...
            <div className="flex flex-col items-end gap-1">
              <span className={`text-[9px] uppercase font-bold tracking-wider flex items-center gap-1 ${session.status === 'Sắp diễn ra' ? 'text-rose-500' : 'text-emerald-500'}`}>
                <span className={`w-1.5 h-1.5 rounded-full ${session.status === 'Sắp diễn ra' ? 'bg-rose-500 animate-pulse' : 'bg-emerald-500'}`} />
                {session.status}
              </span>
