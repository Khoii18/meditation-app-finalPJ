...
export function AiCoachView() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setPlan({
        title: "Lộ Trình Thư Giãn Sâu",
        description: `Tôi thấy bạn đang cảm thấy "${mood}". Hãy dành ra 20 phút để thiết lập lại sự cân bằng. Lộ trình này kết hợp nhịp thở 4-7-8 và hình dung để thả lỏng cơ bắp.`,
        steps: [
          { name: "Khởi động Tâm trí", duration: "5 min", type: "Thở đều" },
          { name: "Quên đi muộn phiền", duration: "10 min", type: "Thiền quét cơ thể" },
          { name: "Cân bằng sóng não", duration: "5 min", type: "Thư giãn âm thanh" },
        ]
      });
      setLoading(false);
    }, 2000);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 md:px-8 pt-12 pb-24">
      <header className="mb-12 text-center max-w-2xl mx-auto">
        ...
        <p className="text-slate-500 dark:text-slate-400">Trí tuệ nhân tạo sẽ tạo một phác đồ thiền định chuẩn xác dựa trên cảm xúc và nhịp sinh học hiện tại của bạn.</p>
      </header>
        ...
        >
          <form onSubmit={handleGenerate}>
            ...
            <textarea 
              ...
              placeholder="Ví dụ: Tôi đang cảm thấy rất áp lực về công việc và khó ngủ..."
            ...
            >
              {loading ? (
                <><Loader2 className="w-5 h-5 animate-spin" /> Đang thiết kế lộ trình...</>
              ) : (
                <><Sparkles className="w-5 h-5" /> Lên Kế Hoạch Ngay</>
              )}
            </button>
          </form>
        </motion.div>
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <div className="flex gap-4">
              <button className="bg-white text-indigo-900 px-8 py-4 rounded-full font-bold flex items-center gap-2 hover:scale-105 transition-transform shadow-xl shadow-white/10">
                <PlayCircle className="w-6 h-6" /> Bắt đầu Tập luyện
              </button>
