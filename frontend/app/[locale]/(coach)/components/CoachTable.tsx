...
export function CoachTable({ data, activeTab, onEdit, onDelete }: { data: any[], activeTab: string, onEdit: (item: any) => void, onDelete: (id: string) => void }) {
  return (
      ...
      <table className="w-full text-left">
        <thead>
          <tr className="border-b border-slate-100 dark:border-white/10">
            ...
            <th className="py-4 font-medium text-slate-500">{activeTab === 'content' ? 'Image / Category' : 'Instructor'}</th>
          ...
          {data.map((item: any) => (
            <tr key={item._id} className="border-b border-slate-50 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
              ...
              <td className="py-4 text-sm text-slate-500">
                ...
                {activeTab === 'content' ? item.type : item.instructor}
              </td>
