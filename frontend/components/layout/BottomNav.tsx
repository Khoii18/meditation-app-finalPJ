...
export function BottomNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-0 inset-x-0 z-50">
      <nav className="w-full bg-surface/95 backdrop-blur-xl border-t border-border px-2 py-2 flex justify-between items-center shadow-[0_-4px_20px_rgba(0,0,0,0.04)] transition-colors duration-500">
  ...
  return (
    <Link href={href} className="flex-1">
      <div className={`flex flex-col items-center gap-1 py-1.5 rounded-xl mx-0.5 transition-colors ${
        isActive ? "text-teal-600 dark:text-teal-400 bg-teal-500/10" : "text-muted hover:text-foreground opacity-70"
      }`}>
        {icon}
